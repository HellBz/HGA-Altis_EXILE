				...
				if (((AGLtoASL _position) distance (getPosASL _x)) < _radius) then
				{
					...
					if (_playerUID in _buildRights) then
					{
						...
						if (_constructionConfigName isEqualTo "Safe") then{
							...
							//["ErrorTitleAndText", [format["Safes: %1, Name: %2, Radius: %3, Position: %4",( count _nearestSafe),_constructionConfigName,_territoryRadius,(getPosASL _x)]]] call ExileClient_gui_toaster_addTemplateToast;
							//systemchat format["Safes: %1, Name: %2, Radius: %3, Position: %4",( count _nearestSafe),_constructionConfigName,_territoryRadius,(getPosASL _x)];
