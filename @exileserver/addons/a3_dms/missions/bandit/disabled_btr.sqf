...
diag_log text format["[DMS DYNAMIC] DISABLED BTR mission has started."];
...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION disabled_btr.sqf with invalid parameters: %1",_this];
};
...
_building = createVehicle ["Land_Cargo_HQ_V1_F",[_pos,10+(random 5),_rndDir+90] call DMS_fnc_SelectOffsetPos,[], 0, "CAN_COLLIDE"];
...
_msgStart = ['#FFFF00',"Bandit's BTR has broken down and needs repair. Eliminate them and take the vehicle."];

// Define Mission Win message
_msgWIN = ['#0080ff',"The BTR has been secured and the bandits eliminated."];

// Define Mission Lose message
_msgLOSE = ['#FF0000',"The bandits have repaired their BTR and escaped."];

// Define mission name (for map marker and logging)
_missionName = "Disabled BTR";
